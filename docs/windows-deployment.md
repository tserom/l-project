...
产物：`dist/stock-inventory-windows.zip`（结构与 **3.1** 一致，含默认 `.env` 与 `README.txt`）。

Mac 包：

```bash
make pack-mac
# 产物：dist/stock-inventory-mac.zip
```
...
| `Makefile` | `build-all` / `pack-mac` / `pack-windows` |
| `scripts/pack/pack-windows.sh` | Windows 运行时 zip 打包 |
| `scripts/pack/pack-mac.sh` | macOS 运行时 zip 打包 |
| `README.md` | 仓库总览与打包入口 |
