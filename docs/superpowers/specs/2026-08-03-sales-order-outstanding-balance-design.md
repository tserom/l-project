...
**日期**：2026-08-03  
**状态**：已实现；实现计划见 `docs/superpowers/plans/2026-08-03-sales-order-outstanding-balance.md`  
...
| 列表 | 本轮不加「往来欠款」列 |
| 历史数据 | **完全不动**：不迁移、不回填、不解析旧备注、不做数据脚本 |
...
- 列表筛选、导出、统计报表
- 任何历史数据处理（含：迁移 IndexedDB、解析旧 `remark` 里的「往来欠款：…」、批量回填）
...
| 历史单 | 不读写、不改造既有记录；缺字段即未填（语言默认行为，非专门兼容逻辑） |
