...
const selected = ref<OutboundOrder | null>(null)
...
async function load() {
  ...
  error.value = ''
  try {
    const res = await listOutboundOrders({ page: 1, pageSize: 50 })
...
async function viewDetail(id: number) {
  try {
    selected.value = await getOutboundOrder(id)
  } catch (e) {
...
async function submit() {
  ...
  try {
    ...
    await createOutboundOrder({ operator: form.operator, remark: form.remark, lines })
...
<template>
  <section class="page">
    ...
    <div class="page__toolbar">
      <button class="btn btn--primary" type="button" @click="showModal = true">新建出库单</button>
