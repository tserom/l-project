...
// OutboundOrder records a draft or confirmed outbound stock document.
type OutboundOrder struct {
	...
	Lines     []OutboundOrderLine `gorm:"foreignKey:OutboundOrderID" json:"lines,omitempty"`
}

func (OutboundOrder) TableName() string { return "outbound_order" }

// OutboundOrderLine is a single outbound line referencing center material and batch.
type OutboundOrderLine struct {
	ID              uint64          `gorm:"primaryKey;autoIncrement" json:"id"`
	OutboundOrderID uint64          `gorm:"not null;index" json:"outboundOrderId"`
